...
df2a=read_csv("data/GWAS/GWAS_Leaf_area_potential_well_watered_LM.csv")
df2a <- df2a[df2a$maf > 0.1, ]
df2b=read_csv("data/GWAS/GWAS_Leaf_area_potential_drought_LM.csv")
...
phenotypesselected<- select(phenotypes, GWAS_Leaf_area_potential_well_watered_LM)
...
outputA <- outputA %>% 
  rename(
    LA_WW_rs = GWAS_Leaf_area_potential_well_watered_LM.x,
    LA_WW_P_val = GWAS_Leaf_area_potential_well_watered_LM.y)
...
phenotypesselected<- select(phenotypes, GWAS_Leaf_area_potential_drought_LM)
accession_id1<-phenotypes$accession_id
selected <-cbind(accession_id1, phenotypesselected)
head(selected)
transposedfinal = read_csv("data/transposedfinal.csv")
...
outputB <- outputB %>% 
  rename(
    LA_LW_rs = GWAS_Leaf_area_potential_drought_LM.x,
    LA_LW_P_val = GWAS_Leaf_area_potential_drought_LM.y)
...
######
#########
#df4 <-filter(df4, ((GWAS_Leaf_area_potential_well_watered_LM > 0.399 & GWAS_Leaf_area_potential_drought_LM < 0.399)| (GWAS_Leaf_area_potential_well_watered_LM < -0.399 & GWAS_Leaf_area_potential_drought_LM > -0.399)| (GWAS_Leaf_area_potential_drought_LM > 0.399 & GWAS_Leaf_area_potential_well_watered_LM < 0.399)| (GWAS_Leaf_area_potential_drought_LM < -0.399 & GWAS_Leaf_area_potential_well_watered_LM > -0.399)))
...
png("Figure S1.png", width = 7, height = 12, units = 'in', res = 350)
plot_grid(rasterGrob(M5),rasterGrob(M4),labels=c('A', 'B'), greedy = TRUE, ncol =1, nrow = 2, align = 'v', hjust=0, label_size=45)
dev.off()
